package com.example.atividade.farmacia.service;
...
import com.example.atividade.farmacia.model.UsuarioLogin;
import com.example.atividade.farmacia.repository.UsuarioRepository;
		...
		if (usuarioRepository.findById(usuario.getId()).isPresent()) {
			...
			if (buscaUsuario.isPresent()) {
				if (buscaUsuario.get().getId() != usuario.getId())
					throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Usúario já existe!", null);
			}
			usuario.setSenha(criptografarSenha(usuario.getSenha()));
			return Optional.of(usuarioRepository.save(usuario));
		}
